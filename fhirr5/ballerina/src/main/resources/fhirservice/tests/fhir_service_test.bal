...
function testCreateConditional412() returns error? {
    ...
    map<string> headers = {
        "If-None-Exist": "http://localhost:9292/fhir/r5/Patient" // not ID for 412
    };
    http:Response|ServiceTestError response = check fhirClient->post("/Patient", message = patient, headers = headers, mediaType = r5:FHIR_MIME_TYPE_JSON);
    test:assertTrue(response is http:Response);
    if response is http:Response {
...
function testCreateConditionalExistingResource() returns error? {
    ...
    map<string> headers = {
        "If-None-Exist": "http://localhost:9292/fhir/r5/Patient?_id=1"
    };
    http:Response|ServiceTestError response = check fhirClient->post("/Patient", message = patient, headers = headers, mediaType = r5:FHIR_MIME_TYPE_JSON);
    test:assertTrue(response is http:Response);
    if response is http:Response {
...
function testCreateConditionalSuccess() returns error? {
    ...
    map<string> headers = {
        "If-None-Exist": "http://localhost:9292/fhir/r5/Patient?_id=New"
    };
    http:Response|ServiceTestError response = check fhirClient->post("/Patient", message = patient, headers = headers, mediaType = r5:FHIR_MIME_TYPE_JSON);
    test:assertTrue(response is http:Response);
    if response is http:Response {
