...
public isolated class FHIRPreprocessor {
    ...
    public isolated function init(r5:ResourceAPIConfig apiConfig) {
        ...
        foreach r5:CommonSearchParameterDefinition item in r5:COMMON_SEARCH_PARAMETERS {
            r5:SearchParamConfig searchParamConfig = {
                name: item.name,
                active: true
            };
        ...
        foreach r5:SearchParamConfig item in self.apiConfig.searchParameters {
            ...
            r5:FHIRSearchParameterDefinition? searchParamDef = r5:fhirRegistry.getResourceSearchParameterByName(self.apiConfig.resourceType, item.name);
            if searchParamDef is () {
                r5:FHIRSearchParameterDefinition customSearchParamDef = {
                    name: item.name,
                    'type: (item.'type != () ? <r5:FHIRSearchParameterType>item.'type : r5:STRING),
                    expression: item.expression,
                    base: []
                };
                r5:fhirRegistry.addSearchParameter(self.apiConfig.resourceType, customSearchParamDef);
            }
        }
        ...
        foreach r5:FHIROperationDefinition operationDefinition in r5:BASE_RESOURCE_OPERATIONS {
            // Operation params
            r5:FHIROperationParameterDefinition[]? 'parameter = operationDefinition.'parameter;
            r5:OperationParamConfig[]? operationParams = 'parameter != () ? 'parameter.map(
                isolated function(r5:FHIROperationParameterDefinition param) returns r5:OperationParamConfig {
                    return {name: param.name, active: true};
                }) : ();
            r5:OperationConfig operationConfig = {
    ...
    public isolated function processRead(string fhirResourceType, string id, http:Request httpRequest, http:RequestContext httpCtx)
                                                                                    returns r5:FHIRError? {
        log:printDebug("Pre-processing FHIR interaction : read");
        // Validate main HTTP headers
        r5:FHIRRequestMimeHeaders clientHeaders = check validateClientRequestHeaders(httpRequest);

        // Create interaction
        readonly & FHIRReadInteraction readInteraction = {id: id};

        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (readInteraction, fhirResourceType, (), {}, clientHeaders.acceptType);

        // Populate JWT information in FHIR context
        readonly & r5:FHIRSecurity fhirSecurity = check getFHIRSecurity(httpRequest);
        ...
        r5:HTTPRequest & readonly request = createHTTPRequestRecord(httpRequest, ());

        // Create FHIR context
        r5:FHIRContext fCtx = new (fhirRequest, request, fhirSecurity);
    ...
    public isolated function processCreate(string resourceType, json|xml payload, http:Request httpRequest,
            http:RequestContext httpCtx) returns r5:FHIRError? {
        ...
        if self.apiConfig.resourceType != resourceType {
            ...
            return r5:createInternalFHIRError("API resource type and API config does not match", r5:ERROR, r5:PROCESSING, diagnostic = diagMsg);
        }
        ...
        r5:FHIRResourceEntity resourceEntity = new (parsedResource);

        // Create interaction
        readonly & FHIRCreateInteraction createInteraction = {};

        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (createInteraction, resourceType, resourceEntity, {}, clientHeaders.acceptType);

        // Populate JWT information in FHIR context
        readonly & r5:FHIRSecurity fhirSecurity = check getFHIRSecurity(httpRequest);

        r5:HTTPRequest & readonly request = createHTTPRequestRecord(httpRequest, payload);

        // Create FHIR context
        r5:FHIRContext fhirCtx = new (fhirRequest, request, fhirSecurity);

        string|error isNoneExistHeader = httpRequest.getHeader("If-None-Exist");
        if isNoneExistHeader is string {
            // conditional create
            // Implemented according to the https://hl7.org/fhir/R5/http.html#ccreate FHIR specification
    ...
    public isolated function processInstanceHistory(string fhirResourceType, string id, http:Request httpRequest, http:RequestContext httpCtx)
        ...
        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (historyInteraction, fhirResourceType, (), {}, clientHeaders.acceptType);
    ...
    public isolated function processVread(string fhirResourceType, string id, string vid, http:Request httpRequest, http:RequestContext httpCtx)
        ...
        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (vreadInteraction, fhirResourceType, (), {}, clientHeaders.acceptType);
    ...
    public isolated function processHistory(http:Request httpRequest, http:RequestContext httpCtx)
        ...
        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (historyInteraction, (), (), {}, clientHeaders.acceptType);
    ...
    public isolated function processCapability(http:Request httpRequest, http:RequestContext httpCtx)
        ...
        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (capabilitiesInteraction, (), (), {}, clientHeaders.acceptType);
    ...
    public isolated function processUpdate(string fhirResourceType, string id, json payload, http:Request httpRequest, http:RequestContext httpCtx)
        ...
        if idInPayload is error {
            return r5:createFHIRError("Payload doesn't have the mandatory ID field", r5:ERROR, r5:PROCESSING,
                    errorType = r5:VALIDATION_ERROR, httpStatusCode = http:STATUS_BAD_REQUEST);
        } else {
            if idInPayload.toString() != id {
                return r5:createFHIRError("Payload ID doesn't match with the resource ID", r5:ERROR, r5:PROCESSING,
                        errorType = r5:VALIDATION_ERROR, httpStatusCode = http:STATUS_BAD_REQUEST);
            }
        }

        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (updateInteraction, fhirResourceType, resourceEntity, {}, clientHeaders.acceptType);
    ...
    public isolated function processPatch(string fhirResourceType, string id, json payload, http:Request httpRequest, http:RequestContext httpCtx)
        ...
        // Validate main HTTP headers
        r5:FHIRRequestMimeHeaders clientHeaders = check validateClientPatchRequestHeaders(httpRequest);
        ...
        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (patchInteraction, fhirResourceType, (), {}, clientHeaders.acceptType);
    ...
    public isolated function processDelete(string fhirResourceType, string id, http:Request httpRequest, http:RequestContext httpCtx)
        ...
        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (deleteInteraction, fhirResourceType, (), {}, clientHeaders.acceptType);
    ...
    # + return - A `r5:FHIRError` if an error occurs during the processing, or `()` otherwise
    public isolated function processOperation(string fhirResourceType, string fhirOperation,
            r5:FHIRInteractionLevel operationRequestScope, json|xml? payload, http:Request httpRequest,
            http:RequestContext httpCtx) returns r5:FHIRError? {
        ...
        r5:FHIRResourceEntity? resourceEntity = ();
        map<r5:RequestSearchParameter[]> requestOperationSearchParameters = {};

        // Get operation definitions for the resource type
        r5:OperationCollection resourceOperationDefinitions = r5:fhirRegistry.getResourceOperations(fhirResourceType);
        ...
        if resourceOperationDefinitions.hasKey(fhirOperation) { // Valid operation for the resource type 
            ...
            r5:FHIROperationDefinition resourceOperationDefinition = resourceOperationDefinitions.get(fhirOperation);
            r5:OperationConfig? resourceOperationConfig = self.operationConfigMap.get(fhirOperation);

            map<r5:RequestSearchParameter[]>|r5:FHIRResourceEntity? processRes =
            ...
            if processRes is map<r5:RequestSearchParameter[]> { // GET invoked
                requestOperationSearchParameters = processRes;
            } else if processRes is r5:FHIRResourceEntity { // POST invoked
                resourceEntity = processRes;
            }
        } else if r5:BASE_RESOURCE_OPERATIONS.hasKey(fhirOperation) { // Base operation (common operation)
            log:printDebug(string `Processing base operation: ${fhirOperation}`);

            // Get base operation definition and config
            r5:FHIROperationDefinition baseOperationDefinition = r5:BASE_RESOURCE_OPERATIONS.get(fhirOperation);
            r5:OperationConfig baseOperationConfig = self.operationConfigMap.get(fhirOperation);
            ...
            if processRes is map<r5:RequestSearchParameter[]> {
                requestOperationSearchParameters = processRes;
            } else if processRes is r5:FHIRResourceEntity {
                resourceEntity = processRes;
            }
        } else { // Invalid operation for the resource type
            string message = "Unknown operation";
            string diagnostic = string `Operation \"$" + fhirOperation + "\" is not known for resource type \""
                    + fhirResourceType + "\". Known and defined operations for \"" + fhirResourceType
                    + "\" resource: " + resourceOperationDefinitions.keys().toString() + ".`;
            return r5:createFHIRError(message, r5:ERROR, r5:PROCESSING, diagnostic = diagnostic,
                    httpStatusCode = http:STATUS_BAD_REQUEST);
        }

        // Create interaction
        readonly & r5:FHIROperationInteraction operationInteraction = {operation: fhirOperation};

        // Create FHIR request
        r5:FHIRRequest fhirRequest = new (operationInteraction, fhirResourceType,
    ...
    isolated function processSearchParameters(string fhirResourceType, http:Request request)
                                                                returns map<r5:RequestSearchParameter[]>|r5:FHIRError {
        map<r5:RequestSearchParameter[]> processedSearchParams = {};
        r5:SearchParamCollection searchParamDefinitions = r5:fhirRegistry.getResourceSearchParameters(fhirResourceType);
        ...
        if request.method == http:POST && request.getContentType().equalsIgnoreCaseAscii("application/x-www-form-urlencoded") {
            ...
            if formData is http:ClientError {
                return r5:createFHIRError("Error occurred while extracting form data", r5:ERROR, r5:PROCESSING,
                        httpStatusCode = http:STATUS_BAD_REQUEST);
            }
        ...
        foreach string originalParamName in requestQueryParams.keys() {
            ...
            r5:RequestQueryParameter queryParam =
                            check r5:decodeSearchParameterKey(originalParamName, requestQueryParams.get(originalParamName));
            r5:RequestSearchParameter[] processResult;
            if searchParamDefinitions.hasKey(queryParam.name) {
                ...
                if self.searchParamConfigMap.hasKey(queryParam.name) && self.searchParamConfigMap.get(queryParam.name).active {
                    ...
                    r5:SearchParamConfig & readonly searchParamConfig = self.searchParamConfigMap.get(queryParam.name);
                    r5:FHIRSearchParameterDefinition parameterDef = searchParamDefinitions.get(queryParam.name);
                    ...
                    string diagnose = string `Unsupported search parameter ${queryParam.name} for resource type 
                        ...
                        ${r5:extractActiveSearchParameterNames(self.searchParamConfigMap).toBalString()}`;
                    return r5:createFHIRError(string `Unsupported search parameter : ${queryParam.name}`, r5:ERROR, r5:PROCESSING,
                            diagnostic = diagnose, httpStatusCode = http:STATUS_BAD_REQUEST);
                }
            } else if r5:COMMON_SEARCH_PARAMETERS.hasKey(queryParam.name) {
                // processing common search parameter
                log:printDebug(string `Processing common search parameter: ${queryParam.name}`);
                r5:CommonSearchParameterDefinition parameterDef = r5:COMMON_SEARCH_PARAMETERS.get(queryParam.name);
            ...
            } else if r5:CONTROL_SEARCH_PARAMETERS.hasKey(queryParam.name) {
                // processing control search parameter
                log:printDebug(string `Processing control search parameter: ${queryParam.name}`);
                r5:CommonSearchParameterDefinition parameterDef = r5:CONTROL_SEARCH_PARAMETERS.get(queryParam.name);
                ...
                return r5:createFHIRError(string `Unknown search parameter : ${queryParam.name}`, r5:ERROR, r5:PROCESSING,
                        diagnostic = diagnose, httpStatusCode = http:STATUS_BAD_REQUEST);
            }
            ...
            if processResult.length() > 0 {
                if processedSearchParams.hasKey(queryParam.name) {
                    r5:RequestSearchParameter[] paramArray = processedSearchParams.get(queryParam.name);
                    foreach r5:RequestSearchParameter item in processResult {
        ...
        foreach r5:CommonSearchParameterDefinition sParam in r5:COMMON_SEARCH_PARAMETERS {
            if !processedSearchParams.hasKey(sParam.name) {
                r5:RequestSearchParameter? defaultParam = check getCommonSearchParamDefault(sParam, self.apiConfig);
                if defaultParam != () {
        ...
        foreach r5:CommonSearchParameterDefinition sParam in r5:CONTROL_SEARCH_PARAMETERS {
            if !processedSearchParams.hasKey(sParam.name) {
                r5:RequestSearchParameter? defaultParam = check getCommonSearchParamDefault(sParam, self.apiConfig);
                if defaultParam != () {
                    processedSearchParams[sParam.name] = [defaultParam];
    ...
    isolated function processPaginationParams(http:Request request) returns r5:PaginationContext|r5:FHIRError {
        int page = 1;
        ...
        if request.getQueryParamValue(PAGE) is string {
            if self.paginationEnabled {
                ...
                if pageStr is string {
                    ...
                    if pageInt is error {
                        return r5:createFHIRError(string `Invalid page number: ${pageStr}`, r5:ERROR, r5:PROCESSING, httpStatusCode = http:STATUS_BAD_REQUEST);
                    } else {
                        page = pageInt;
                    }
                }
                ...
                return r5:createFHIRError("Pagination not supported", r5:ERROR, r5:PROCESSING, httpStatusCode = http:STATUS_BAD_REQUEST);
            }
        }
            ...
            if self.paginationEnabled {
                ...
                if countStr is string {
                    ...
                    if countInt is error {
                        return r5:createFHIRError(string `Invalid count value: ${countStr}`, r5:ERROR, r5:PROCESSING, httpStatusCode = http:STATUS_BAD_REQUEST);
                    } else {
    ...
    isolated function handleSmartSecurity(r5:FHIRSecurity fhirSecurity, string? id) returns r5:FHIRError? {
        r5:AuthzConfig? authzConfig = self.apiConfig.authzConfig;
        if authzConfig is r5:AuthzConfig {
            _ = check r5:handleSmartSecurity(authzConfig, fhirSecurity, id);
        }
    }
}
...
# + return - A map of processed request search parameters if the operation is a GET request, a `r5:FHIRResourceEntity` 
# if the operation is a POST request with a payload, a `r5:FHIRError` if an error occurs during the pre-processing, or 
...
isolated function preProcessOperation(string fhirResourceType, string fhirOperation,
        r5:FHIRInteractionLevel operationRequestScope, r5:FHIROperationDefinition operationDefinition,
        r5:OperationConfig? operationConfig, map<r5:OperationConfig> operationConfigMap, r5:ResourceAPIConfig apiConfig,
        json|xml? payload, http:Request httpRequest, http:RequestContext httpCtx, r5:FHIRRequestMimeHeaders clientHeaders)
        returns map<r5:RequestSearchParameter[]>|r5:FHIRResourceEntity|r5:FHIRError? {
    ...
    r5:FHIROperationParameterDefinition[] operationParameterDefinitions = operationDefinition.'parameter ?: [];
    r5:OperationParamConfig[] operationParameterConfigs = operationConfig.parameters ?: [];

    // An operation can be GET or POST invoked
    if httpRequest.method == http:GET {
        // Custom pre-processor check
        r5:OperationPreProcessor? customPreProcessor = operationConfig.preProcessor;
        ...
        anydata|r5:FHIRParseError parsedResource = parser:parse(payload);
        international500:Parameters|error parsedParametersPayload = parsedResource.ensureType();

        if parsedResource is r5:FHIRParseError || parsedParametersPayload is error {
        ...
        map<international500:ParametersParameter[]> processedParams = check processOperationPayloadParams(fhirOperation,
        ...
        return new r5:FHIRResourceEntity(parsedResource);
    } else {
...
# + return - A map of processed operation search parameters, or a `r5:FHIRError` if an error occurs during the processing
isolated function processOperationQueryParams(string fhirOperation,
        r5:FHIRInteractionLevel operationRequestScope,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions,
        r5:OperationParamConfig[] operationParamConfigs, http:Request httpRequest)
    returns map<r5:RequestSearchParameter[]>|r5:FHIRError {
    log:printDebug(string `Processing GET invoked operation params of operation: ${fhirOperation}`);

    map<r5:RequestSearchParameter[]> processedOperationSearchParams = {};
    ...
    foreach string originalParamName in requestQueryParams.keys() {
        ...
        r5:RequestQueryParameter queryParam = check r5:decodeSearchParameterKey(originalParamName, paramValues);
...
# + return - A `r5:FHIRError` if an error occurs during the processing, or `()` if the processing is successful
isolated function processOperationQueryParam(r5:RequestQueryParameter queryParam, string fhirOperation,
        r5:FHIRInteractionLevel operationRequestScope, r5:FHIROperationParameterDefinition[] operationParamDefinitions,
        r5:OperationParamConfig[] operationParamConfigs, map<r5:RequestSearchParameter[]> processedOperationSearchParams)
        returns r5:FHIRError? {
    ...
    r5:FHIROperationParameterDefinition? paramDefinition = getOperationParameterDefinition(paramName,
    ...
    r5:OperationParamConfig? paramConfig = getOperationParameterConfig(paramName, operationParamConfigs);
    ...
    r5:RequestSearchParameter[] searchParams;
...
# + operationParamDefinitions - The operation parameter definitions
# + operationParamConfigs - The operation parameter configurations
# + return - An array of processed search parameters, or a `r5:FHIRError` if an error occurs during the processing
isolated function processOperationPartQueryParam(string fhirOperation, r5:FHIRInteractionLevel operationRequestScope,
        r5:RequestQueryParameter queryParam, string partParamName, string[] remainingParts,
        r5:FHIROperationParameterDefinition currentParamDefinition, r5:OperationParamConfig currentParamConfig,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions,
        r5:OperationParamConfig[] operationParamConfigs) returns r5:RequestSearchParameter[]|r5:FHIRError {
    // Get part parameter definition
    r5:FHIROperationParameterDefinition? partParamDefinition =
    ...
    r5:OperationParamConfig? partParamConfig = getOperationPartParameterConfig(partParamName, currentParamConfig);
    ...
    // For an example, see https://hl7.org/fhir/R5/codesystem-operation-find-matches.html
...
# or a `r5:FHIRError` otherwise
isolated function processOperationQueryParamAsSearchParam(r5:RequestQueryParameter queryParam,
        r5:FHIROperationParameterDefinition paramDefinition) returns r5:RequestSearchParameter[]|r5:FHIRError {
    r5:FHIROperationParameterDefinition tmpParamDefinition;
    ...
    if paramDefinition.searchType != () {
        ...
        r5:FHIRSearchParameterType|r5:FHIRError searchParamType = r5:primitiveTypeToSearchParamType('type);
        if searchParamType is r5:FHIRError { // Parameter type is not a primitive type
            string message = "Invalid operation invocation";
        ...
        r5:FHIROperationParameterDefinition newDefinition = {
    ...
    foreach string value in queryParam.values {
        r5:RequestSearchParameter searchParam =
                    check r5:createRequestSearchParameter(tmpParamDefinition, queryParam.modifier, value);
...
# + return - A map of processed parameters, or a `r5:FHIRError` if a parameter cannot be processed
isolated function processOperationPayloadParams(string fhirOperation, r5:FHIRInteractionLevel operationRequestScope,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions, r5:OperationParamConfig[] operationParamConfigs,
        international500:Parameters parsedParametersPayload) returns map<international500:ParametersParameter[]>|r5:FHIRError {
    log:printDebug(string `Processing POST invoked operation params of operation: ${fhirOperation}`);
    map<international500:ParametersParameter[]> processedOperationParams = {};

    international500:ParametersParameter[]? parameters = parsedParametersPayload.'parameter;
    if parameters != () {
        foreach international500:ParametersParameter 'parameter in parameters {
...
# + return - A `r5:FHIRError` if the parameter cannot be processed, nil otherwise
isolated function validateAndProcessOperationPayloadParam(string fhirOperation,
        international500:ParametersParameter 'parameter, r5:FHIRInteractionLevel operationRequestScope,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions, r5:OperationParamConfig[] operationParamConfigs,
        map<international500:ParametersParameter[]> processedOperationParams) returns r5:FHIRError? {
    // Get parameter definition and config
    r5:FHIROperationParameterDefinition? parameterDefinition = getOperationParameterDefinition('parameter.name,
            operationParamDefinitions);
    r5:OperationParamConfig? parameterConfig = getOperationParameterConfig('parameter.name, operationParamConfigs);
...
# + operationParamConfigs - The parameter configurations for the operation
# + processedOperationParams - The map of processed parameters
# + return - A map of processed parameters, or a `r5:FHIRError` if the part parameter cannot be processed
isolated function validateAndProcessOperationPayloadPartParam(string fhirOperation,
        international500:ParametersParameter 'parameter, string fullParamName, r5:FHIRInteractionLevel operationRequestScope,
        r5:FHIROperationParameterDefinition currentParamDefinition, r5:OperationParamConfig currentParamConfig,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions, r5:OperationParamConfig[] operationParamConfigs,
        map<international500:ParametersParameter[]> processedOperationParams) returns r5:FHIRError? {
    // Get part parameter definition and config
    r5:FHIROperationParameterDefinition? partParameterDefinition = getOperationPartParameterDefinition('parameter.name,
            currentParamDefinition);
    r5:OperationParamConfig? partParameterConfig = getOperationPartParameterConfig('parameter.name, currentParamConfig);
...
# + operationParamConfigs - The parameter configurations for the operation
# + return - A `r5:FHIRError` if the parameter cannot be processed, nil otherwise
isolated function processOperationPayloadParam(string fhirOperation, international500:ParametersParameter 'parameter,
        string paramName, r5:FHIRInteractionLevel operationRequestScope,
        r5:FHIROperationParameterDefinition? parameterDefinition, r5:OperationParamConfig? parameterConfig,
        map<international500:ParametersParameter[]> processedOperationParams,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions,
        r5:OperationParamConfig[] operationParamConfigs) returns r5:FHIRError? {
    ...
    if isMultiPartOperationParameter(parameterDefinition) {
        international500:ParametersParameter[]? paramParts = 'parameter.part;
        if paramParts != () {
            foreach international500:ParametersParameter partParam in paramParts {
...
# + return - A `r5:FHIRError` if the parameter does not support the request operation scope, nil otherwise
isolated function validateOperationParamScope(string fhirOperation, r5:OperationParamConfig paramConfig,
        r5:FHIRInteractionLevel requestOperationScope) returns r5:FHIRError? {
    // 'scope' element is not defined for R5 operation definitions. Therefore, we validate scope 
    ...
    r5:FHIRInteractionLevel[]? supportedScopes = paramConfig.scopes;
...
# + return - A `r5:FHIRError` if the cardinality of a parameter is not valid, nil otherwise
isolated function validateOperationParamCardinality(string fhirOperation,
        r5:OperationParamConfig[] operationParamConfigs,
        map<r5:RequestSearchParameter[]>|map<international500:ParametersParameter[]> parameters,
        string? multiPartParam = ()) returns r5:FHIRError? {
    // Cardinality could vary based on the parameter's scope. However, as the 'scope' property 
    // is not defined in 5 operation definitions, we validate the cardinality defined in the API config.
    foreach r5:OperationParamConfig paramConfig in operationParamConfigs {
        // Get cardinalities
        var [minCardinality, maxCardinality] = getOperationParamCardinalities(paramConfig);

        // Get parameter parts
        r5:OperationParamConfig[]? partParamConfigs = paramConfig.parts;
...
isolated function getOperationParamCardinalities(r5:FHIROperationParameterDefinition|r5:OperationParamConfig 'parameter)
        returns [int, int] {
...
isolated function isStateAffectedByOperation(r5:FHIROperationDefinition operationDefinition) returns boolean =>
...
isolated function isMultiPartOperationParameter(r5:FHIROperationParameterDefinition
        |r5:OperationParamConfig operationParameter) returns boolean {
    if operationParameter is r5:FHIROperationParameterDefinition {
...
isolated function setFHIRContext(r5:FHIRContext ctx, http:RequestContext httpCtx) {
    httpCtx.set(r5:FHIR_CONTEXT_PROP_NAME, ctx);
}

isolated function getNextService(http:RequestContext httpCtx) returns http:NextService?|r5:FHIRError {
    http:NextService|error? next = httpCtx.next();
    if next is error {
        return r5:createInternalFHIRError("Error occurred while retrieving next service", r5:ERROR, r5:PROCESSING, cause = next);
    }
    return next;
}

isolated function processResourceBoundSearchParameter(r5:FHIRSearchParameterDefinition definition,
        string fhirResourceType,
        readonly & r5:SearchParamConfig config,
        r5:RequestQueryParameter queryParam)
                                                                        returns r5:RequestSearchParameter[]|r5:FHIRError {
    r5:SearchParameterPreProcessor? customPreprocessor = config.preProcessor;
    if customPreprocessor != () {
...
isolated function processCommonSearchParameter(r5:CommonSearchParameterDefinition definition,
        string fhirResourceType,
        r5:RequestQueryParameter queryParam,
        r5:ResourceAPIConfig apiConfig,
        map<r5:SearchParamConfig> & readonly searchParamConfigMap)
                                                                        returns r5:RequestSearchParameter[]|r5:FHIRError {
    if searchParamConfigMap.hasKey(definition.name) {
        // The common search parameter behavior is override via Resource API Config
        r5:SearchParamConfig & readonly paramConfig = searchParamConfigMap.get(definition.name);
        if paramConfig.active {
            //Check whether preprocessor is overriden
            readonly & r5:SearchParameterPreProcessor? customPreprocessor = paramConfig?.preProcessor;
            if customPreprocessor != () {
                // Create temporary FHIR Search parameter definition from common search parameter definition
                r5:FHIRSearchParameterDefinition tempParamDef = {
            ...
            string diagnose = string `Unsupported search parameter \"${queryParam.name}\" for resource type \" 
                ${fhirResourceType}\". Supported search parameters are: ${r5:extractActiveSearchParameterNames(searchParamConfigMap).toBalString()}`;
            return r5:createFHIRError(string `Unsupported search parameter : ${queryParam.name}`, r5:ERROR, r5:PROCESSING_NOT_SUPPORTED, diagnostic = diagnose,
                    httpStatusCode = http:STATUS_NOT_IMPLEMENTED);
        }
    }
    // If we reach here, the developer haven't override the search parameter
    r5:CommonSearchParameterPreProcessor? preProcessor = definition.preProcessor;
    if preProcessor != () {
...
isolated function getCommonSearchParamDefault(r5:CommonSearchParameterDefinition definition, r5:ResourceAPIConfig apiConfig)
                                                                        returns r5:RequestSearchParameter|r5:FHIRError? {

    anydata value;
    anydata|r5:SearchParameterDefaultValueProcessor? default = definition.default;
    if default != () {
        if default is r5:SearchParameterDefaultValueProcessor {
            r5:SearchParameterDefaultValueProcessor defaultFn = <r5:SearchParameterDefaultValueProcessor>default;
            value = check defaultFn(definition, apiConfig);
        } else {
            value = default;
        }
        return check r5:createRequestSearchParameter(definition, (), value);
    }
    return ();
}

isolated function preprocessGeneralSearchParameter(r5:FHIRSearchParameterDefinition|r5:CommonSearchParameterDefinition definition,
        r5:RequestQueryParameter queryParam) returns r5:RequestSearchParameter[]|r5:FHIRError {
    r5:RequestSearchParameter[] parameters = [];
    foreach string value in queryParam.values {
        r5:RequestSearchParameter? requestSearchParam =
                                            check r5:createRequestSearchParameter(definition, queryParam.modifier, value);
...
isolated function validateClientRequestHeaders(http:Request request) returns r5:FHIRRequestMimeHeaders|r5:FHIRError {
    r5:FHIRRequestMimeHeaders headers = {};
    ...
    match contentType {
        ...
        _ => {
            ...
            return r5:createFHIRError(message, r5:ERROR, r5:PROCESSING, message, errorType = r5:VALIDATION_ERROR, httpStatusCode = http:STATUS_UNSUPPORTED_MEDIA_TYPE);
        }
    }
    ...
    if acceptHeader is string {
        ...
        if !isValidMimeType {
            string message = string `Unsupported Accept header value of \"${acceptHeader}\" was provided in the request. 
                    Only \"application/fhir+json\" is supported.`;
            return r5:createFHIRError(message, r5:ERROR, r5:PROCESSING, message, errorType = r5:VALIDATION_ERROR, httpStatusCode = http:STATUS_NOT_ACCEPTABLE);
        }
    }
...
# + request - HTTP request object
# + return - FHIRRequestMimeHeaders containing details extracted from about MIME types. FHIRError otherwise
isolated function validateClientPatchRequestHeaders(http:Request request) returns r5:FHIRRequestMimeHeaders|r5:FHIRError {
    r5:FHIRRequestMimeHeaders headers = {};
...
isolated function getJwtDetails(http:Request httpRequest) returns readonly & r5:FHIRSecurity|r5:FHIRError {

    readonly & r5:FHIRSecurity fhirSecurity;
        ...
        if headerPayload is [jwt:Header, jwt:Payload] {
            ...
            if payload.hasKey(IDP_CLAIMS) {
                ...
                if claimList is error {
                    ...
                    return r5:createFHIRError(message, r5:ERROR, r5:PROCESSING, message,
                            errorType = r5:PROCESSING_ERROR, httpStatusCode = http:STATUS_UNAUTHORIZED);
                }
                ...
                if userName is error {
                    ...
                    readonly & r5:FHIRUser fhirUserInfo = {
...
isolated function getFHIRSecurity(http:Request request) returns readonly & r5:FHIRSecurity|r5:FHIRError {

    return getJwtDetails(request);
}

isolated function createHTTPRequestRecord(http:Request request, json|xml? payload) returns readonly & r5:HTTPRequest {
    map<string[]> headers = {};
...
isolated function getOperationParameterDefinition(string paramName,
        r5:FHIROperationParameterDefinition[] parameterDefinitions) returns r5:FHIROperationParameterDefinition? {
    foreach r5:FHIROperationParameterDefinition paramDef in parameterDefinitions {
...
isolated function getOperationPartParameterDefinition(string partParamName,
        r5:FHIROperationParameterDefinition multiPartParamDefinition) returns r5:FHIROperationParameterDefinition? {
    r5:FHIROperationParameterDefinition[]? parts = multiPartParamDefinition.part;
...
isolated function getOperationParameterConfig(string paramName,
        r5:OperationParamConfig[] operationParameterConfigs) returns r5:OperationParamConfig? {
    foreach r5:OperationParamConfig paramConfig in operationParameterConfigs {
...
isolated function getOperationPartParameterConfig(string partParamName,
        r5:OperationParamConfig multiPartParamConfig) returns r5:OperationParamConfig? {
    r5:OperationParamConfig[]? parts = multiPartParamConfig.parts;
...
isolated function extractActiveOperationNames(map<r5:OperationConfig> operationConfigMap) returns string[] {
    string[] activeOperations = [];
    foreach r5:OperationConfig operationConfig in operationConfigMap {
...
isolated function extractKnownOperationInParamNames(r5:FHIROperationParameterDefinition[] paramDefinitions,
        string? multiPartParam = ()) returns string[] {
    string[] knownParameters = [];
    foreach r5:FHIROperationParameterDefinition paramDefinition in paramDefinitions {
        if paramDefinition.use == r5:INPUT { // Only input parameters
            string paramName = multiPartParam != () ? string `${multiPartParam}.${paramDefinition.name}` : paramDefinition.name;
            if isMultiPartOperationParameter(paramDefinition) {
                // Check part parameters
                r5:FHIROperationParameterDefinition[]? partParamDefinitions = paramDefinition.part;
...
# + multiPartParam - The name of the multi-part parameter that the input parameters are part of, if any
# + return - An array containing the names of known input parameters
isolated function extractActiveOperationInParamNames(r5:OperationParamConfig[] paramConfigs,
        string? multiPartParam = ()) returns string[] {
    string[] activeParameters = [];
    foreach r5:OperationParamConfig paramConfig in paramConfigs {
        if paramConfig.active {
            string paramName = multiPartParam != () ? string `${multiPartParam}.${paramConfig.name}` : paramConfig.name;
...
isolated function getOperationScopes(r5:FHIROperationDefinition operationDefinition) returns string[] {
    string[] scopes = [];
    if operationDefinition.systemLevel {
        scopes.push(r5:FHIR_INTERACTION_SYSTEM);
    }
    if operationDefinition.typeLevel {
        scopes.push(r5:FHIR_INTERACTION_TYPE);
    }
    if operationDefinition.instanceLevel {
        scopes.push(r5:FHIR_INTERACTION_INSTANCE);
    }
    return scopes;
}

# Creates a `r5:FHIRError` for an unknown operation parameter.
#
...
# + return - The created `r5:FHIRError`
isolated function createUnknownOperationParamError(string paramName, string fhirOperation,
        r5:FHIROperationParameterDefinition[] operationParamDefinitions) returns r5:FHIRError {
...
# Creates a `r5:FHIRError` for an unsupported operation parameter.
#
...
# Creates a `r5:FHIRError` for a multi-part operation parameter that is missing its parts.
#
