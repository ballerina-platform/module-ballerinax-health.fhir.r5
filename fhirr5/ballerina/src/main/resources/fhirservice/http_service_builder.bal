...
isolated function getHttpService(Holder h, r5:ResourceAPIConfig apiConfig, string[] & readonly servicePath) returns http:Service {
    http:InterceptableService httpService = isolated service object {
            ...
            if resourceMethod is handle {
                ...
                if isOperationPath(paths) {
                    ...
                    if (paths.length() >= 1 && paths[paths.length() - 1] == HISTORY) {
                        ...
                        string fhirResource = apiConfig.resourceType;
                        string id = paths[paths.length() - 2];
                        r5:FHIRError? processIHistory = self.preprocessor.processInstanceHistory(fhirResource, id, req, ctx);
                        if processIHistory is r5:FHIRError {
                            return processIHistory;
                        }
                        fhirContext = check r5:getFHIRContext(ctx);
                        executeResourceResult = executeWithID(id, fhirContext, fhirService, resourceMethod);
                        if executeResourceResult is r5:Bundle {
                        ...
                        r5:FHIRError? processIHistory = self.preprocessor.processVread(fhirResource, id, vid, req, ctx);
                        if processIHistory is r5:FHIRError {
                            return processIHistory;
                        }
                        fhirContext = check r5:getFHIRContext(ctx);
                        ...
                        r5:FHIRError? processRead = self.preprocessor.processRead(fhirResource, id, req, ctx);
                        if processRead is r5:FHIRError {
                            return processRead;
                        }
                        fhirContext = check r5:getFHIRContext(ctx);
                        executeResourceResult = executeWithID(id, fhirContext, fhirService, resourceMethod);
                    ...
                    if (paths.length() >= 1 && paths[paths.length() - 1] == HISTORY) {
                        ...
                        r5:FHIRError? processHistory = self.preprocessor.processHistory(req, ctx);
                        if processHistory is r5:FHIRError {
                        ...
                        r5:FHIRError? processCapability = self.preprocessor.processCapability(req, ctx);
                        if processCapability is r5:FHIRError {
                        ...
                        string fhirResource = apiConfig.resourceType;
                        r5:FHIRError? processSearch = self.preprocessor.processSearch(fhirResource, req, ctx);
                        if processSearch is r5:FHIRError {
                ...
                if (executeResourceResult is error) {
                    ...
                    fhirContext.setErrorCode(r5:getErrorCode(executeResourceResult));
                    return r5:handleErrorResponse(executeResourceResult);
                }
                return executeResourceResult;
            } else {
                return r5:createFHIRError(string `Path not found: ${req.extraPathInfo}`, r5:CODE_SEVERITY_ERROR, r5:TRANSIENT, httpStatusCode = http:STATUS_NOT_FOUND);
            }
        }
            ...
            if resourceMethod is handle {
                if isOperationPath(paths) { // An operation
                    if payload is json || payload is http:NoContentError {
                        ...
                        r5:FHIRInteractionLevel operationScope = getRequestOperationScope(operation,
                                fhirResource, paths);
                        ...
                        return r5:createFHIRError(string `Invalid operation payload`, r5:CODE_SEVERITY_ERROR,
                                r5:TRANSIENT, httpStatusCode = http:STATUS_BAD_REQUEST);
                    }
                    ...
                    r5:FHIRError? processCreate = self.preprocessor.processCreate(fhirResource, payload, req, ctx);
                    if processCreate is r5:FHIRError {
                        return processCreate;
                    }
                    fhirContext = check r5:getFHIRContext(ctx);
                    executeResourceResult = executeWithPayload(payload, fhirContext, fhirService, resourceMethod);
                } else {
                    return r5:createFHIRError(string `Invalid payload`, r5:CODE_SEVERITY_ERROR, r5:TRANSIENT,
                ...
                if executeResourceResult is r5:DomainResource {
                    string? createdId = executeResourceResult.id;
                    if createdId is string {
                        string location = string `${fhirResource}/${createdId}`;
                        fhirContext.setProperty(r5:LOCATION_HEADER_PROP_NAME, location);
                    }
                }
                return executeResourceResult;
            } else {
                return r5:createFHIRError(string `Path not found: ${req.extraPathInfo}`, r5:CODE_SEVERITY_ERROR,
                        r5:TRANSIENT, httpStatusCode = http:STATUS_NOT_FOUND);
            }
        }
        ...
        isolated resource function put [string... path](http:Request req, http:RequestContext ctx) returns any|error {
            ...
            if payload is json {
                if resourceMethod is handle {
                    string fhirResource = apiConfig.resourceType;
                    string id = paths[paths.length() - 1];
                    r5:FHIRError? processUpdate = self.preprocessor.processUpdate(fhirResource, id, payload, req, ctx);
                    if processUpdate is r5:FHIRError {
                        return processUpdate;
                    }
                    r5:FHIRContext fhirContext = check r5:getFHIRContext(ctx);
                ...
                return r5:createFHIRError(string `Invalid payload`, r5:CODE_SEVERITY_ERROR, r5:TRANSIENT, httpStatusCode = http:STATUS_BAD_REQUEST);
            }
        }
        isolated resource function patch [string... path](http:Request req, http:RequestContext ctx) returns any|error {
            ...
            if payload is json {
                if resourceMethod is handle {
                    string fhirResource = apiConfig.resourceType;
                    string id = paths[paths.length() - 1];
                    r5:FHIRError? processPatch = self.preprocessor.processPatch(fhirResource, id, payload, req, ctx);
                    if processPatch is r5:FHIRError {
                        return processPatch;
                    }
                    r5:FHIRContext fhirContext = check r5:getFHIRContext(ctx);
                    any|error executeResourceResult = executeWithIDAndPayload(id, payload, fhirContext, fhirService, resourceMethod);
        ...
        isolated resource function delete [string... path](http:Request req, http:RequestContext ctx) returns any|error {
            ...
            if resourceMethod is handle {
                string fhirResource = apiConfig.resourceType;
                string id = paths[paths.length() - 1];
                r5:FHIRError? processDelete = self.preprocessor.processDelete(fhirResource, id, req, ctx);
                if processDelete is r5:FHIRError {
