...
import static io.ballerinax.fhirr5.Utils.isPathsMatching;
...
public class HTTPToFHIRAdaptor {
    ...
    private static final String MODULE_NAME = "ballerinax/health.fhirr5";
    ...
    public static Object executeWithPayload(Environment environment, BMap r5Payload, BObject fhirCtx, BObject service,
                                       ResourceMethodType resourceMethod) {
        ServiceType serviceType = (ServiceType) service.getType();
        return environment.getRuntime().callMethod(service, resourceMethod.getName(),
                new StrandMetadata(serviceType.isIsolated() && serviceType.isIsolated(resourceMethod.getName()), ModuleUtils.getProperties("executeWithPayload")), fhirCtx, r5Payload);
    }
