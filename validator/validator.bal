...
public isolated function createValidationError(string message, r5:Severity errServerity, r5:IssueType code,
        string? diagnostic = (), string[]? expression = (), error? cause = (),
        r5:FHIRErrorTypes? errorType = (), string[]? parsedErrors = (), int httpStatusCode = http:STATUS_INTERNAL_SERVER_ERROR)
    ...
    match errorType {
        r5:VALIDATION_ERROR => {
            r5:FHIRValidationError fError = error(message, cause, issues = [issue], httpStatusCode = httpStatusCode,
                                                        internalError = internal);
            return fError;
        }
        r5:PARSE_ERROR => {
            r5:FHIRParseError fError = error(message, cause, issues = [issue], httpStatusCode = httpStatusCode,
                                                        internalError = internal);
            return fError;
        }
        _ => {
            r5:FHIRError fError = error(message, cause, issues = [issue], httpStatusCode = httpStatusCode,
                                                        internalError = internal);
            return fError;
        }
    }
...
public isolated function validate(anydata data, typedesc<anydata>? targetFHIRModelType = ()) returns r5:FHIRValidationError? {

    anydata finalData;

    if data is json|xml|string {
        anydata|r5:FHIRParseError parsedResult = parser:parse(data, targetFHIRModelType);

        if parsedResult is r5:FHIRParseError {
            log:printDebug(string `FHIR parsing failed, ${parsedResult.message()}`);
            string[] errors = processFHIRParserErrors(parsedResult.message());
            return <r5:FHIRValidationError>createValidationError("FHIR resource validation failed", r5:ERROR, r5:INVALID, parsedResult.message(),
                                                errorType = r5:VALIDATION_ERROR, cause = parsedResult, parsedErrors = errors, httpStatusCode = parsedResult.detail().httpStatusCode);
        } else {
            finalData = parsedResult;
        }
    } else if data is r5:DomainResource{
        finalData = data;
    } else {
        return <r5:FHIRValidationError>r5:createFHIRError("FHIR resource validation failed", r5:ERROR, r5:INVALID, 
                        "Invalid FHIR resource type", errorType = r5:VALIDATION_ERROR, httpStatusCode = http:STATUS_BAD_REQUEST);
    }
    ...
    if validationResult is constraint:Error {
        ...
        return <r5:FHIRValidationError>createValidationError("FHIR resource validation failed", r5:ERROR, r5:INVALID, validationResult.message(),
                                                errorType = r5:VALIDATION_ERROR, cause = validationResult, parsedErrors = errors, httpStatusCode = http:STATUS_BAD_REQUEST);
    } 
    ...
    if terminologyValidationEnabled {
        ...
        if validationErrors is string[] {    
            return <r5:FHIRValidationError>createValidationError("FHIR resource validation failed, due to terminology validation failed", r5:ERROR, r5:INVALID, "Terminology validation failed", 
                errorType = r5:VALIDATION_ERROR, parsedErrors = validationErrors, httpStatusCode = http:STATUS_BAD_REQUEST);
        } else {
...
isolated function processFHIRParserErrors(string message) returns string[] {
    ...
    // {
    //     missing required field 'x' of type 'health.fhir.r5:CodeableConcept' in record 'x'
